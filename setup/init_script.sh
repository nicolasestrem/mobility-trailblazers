...
# =============================================================================
# STEP 3.5: DEPLOY PLUGIN FILES
# =============================================================================

echo "🔍 Step 3.5: Deploying plugin files..."

PLUGIN_SOURCE="/path/to/mobility-trailblazers-plugin"
PLUGIN_DEST="/mnt/dietpi_userdata/docker-files/STAGING/wordpress_data/wp-content/plugins/mobility-trailblazers"

if [ -d "$PLUGIN_SOURCE" ]; then
    echo "📦 Copying plugin files..."
    sudo cp -r "$PLUGIN_SOURCE" "$PLUGIN_DEST"
    sudo chown -R 33:33 "$PLUGIN_DEST"
...
# =============================================================================
# STEP 3.6: CHECK FOR ELEMENTOR
# =============================================================================

echo "🔍 Step 3.6: Checking for Elementor..."

ELEMENTOR_ACTIVE=$(docker exec mobility_wpcli_STAGING wp plugin is-active elementor --path="/var/www/html" 2>/dev/null && echo "true" || echo "false")

if [ "$ELEMENTOR_ACTIVE" = "false" ]; then
    echo "⚠️  WARNING: Elementor is not active. Some features may not work properly."
    echo "📝 Install Elementor with: docker exec mobility_wpcli_STAGING wp plugin install elementor --activate"
fi
echo ""
...
# =============================================================================
# STEP 6.5: VERIFY DATABASE TABLES
# =============================================================================

echo "🔍 Step 6.5: Verifying database tables..."

TABLES=$(docker exec mobility_mariadb_STAGING mysql -u wp_user -pWp7kL9xP2qR7vN6wE3zY4uC1sA5f wordpress_db -e "SHOW TABLES LIKE '%mt_%';" 2>/dev/null | grep -c "mt_")

echo "📊 Mobility Trailblazers tables found: $TABLES"

EXPECTED_TABLES=("mt_votes" "mt_public_votes" "mt_candidate_scores" "mt_assignments")
for table in "${EXPECTED_TABLES[@]}"; do
    TABLE_EXISTS=$(docker exec mobility_mariadb_STAGING mysql -u wp_user -pWp7kL9xP2qR7vN6wE3zY4uC1sA5f wordpress_db -e "SHOW TABLES LIKE 'wp_$table';" 2>/dev/null | grep -c "$table")
    if [ "$TABLE_EXISTS" = "1" ]; then
        echo "✅ Table wp_$table exists"
    else
        echo "❌ Table wp_$table missing"
    fi
done
echo ""
