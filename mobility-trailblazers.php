...
class MobilityTrailblazersPlugin {
    ...
    private function register_ajax_handlers() {
        ...
        add_action('admin_menu', array($this, 'mt_test_data_page'));
    }
    
    /**
     * Add temporary test data page to admin menu
     */
    public function mt_test_data_page() {
        add_submenu_page(
            'mt-award-system',
            'Test Data',
            'Test Data',
            'manage_options',
            'mt-test-data',
            array($this, 'mt_render_test_data_page')
        );
    }
    
    /**
     * Render test data page content
     */
    public function mt_render_test_data_page() {
        ...
        echo '<pre>' . esc_html(json_encode($this->get_candidates_for_js(), JSON_PRETTY_PRINT)) . '</pre>';
        
        echo '<h3>Jury Members for JS:</h3>';
        echo '<pre>' . esc_html(json_encode($this->get_jury_members_for_js(), JSON_PRETTY_PRINT)) . '</pre>';
