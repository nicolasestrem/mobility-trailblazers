...
function generate_candidate($category, $data_sets, $index) {
    ...
    switch ($category) {
        case 'established-companies':
            ...
            $category_slug = 'etablierte-unternehmen';
            break;
        ...
        case 'startups-new-makers':
            ...
            $category_slug = 'startups-neue-gestalter';
            break;
        ...
        case 'infrastructure-politics-public':
            ...
            $category_slug = 'infrastruktur-politik-oeffentliche';
            break;
    }
            ...
            try {
                ...
                $post_id = wp_insert_post(array(
                    ...
                    'meta_input' => array(
                        ...
                        '_mt_phone' => '', // Add empty phone field
                        '_mt_video_url' => '', // Add empty video field
                        '_mt_nomination_source' => 'Database Import',
                        '_mt_nomination_date' => current_time('mysql')
                    )
                ));
                ...
                // Randomly assign status (weighted towards nominated)
                $status_weights = array(
                    'nominated' => 70,      // 70% nominated
                    'under-review' => 20,   // 20% under review
                    'shortlisted' => 5,     // 5% shortlisted
                    'finalist' => 3,        // 3% finalist
                    'winner' => 1,          // 1% winner
                    'not-selected' => 1     // 1% not selected
                );
                
                $random_num = rand(1, 100);
                $cumulative = 0;
                $selected_status = 'nominated';
